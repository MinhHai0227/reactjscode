...
function ChapterContainerPage() {
  ...
  const chapter = useSelector(
    (state) => state.chapter.chapterByComicId[chapter_id]
  );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="relative">
          ...
          <h2 className="absolute uppercase  bottom-0 inset-x-0 text-center bg-slate-50 border-t-2 border-dashed text-gray-900 lg:text-2xl p-2 h-fit">
            cập nhật chương mới sớm nhất tại website{" "}
        ...
        <div className="bg-white shadow rounded-sm mt-2">
          <ol className="flex gap-3 pt-5 px-3 text-gray-500">
            ...
            <li className="before:mr-3 before:text-gray-500 before:content-['/'] ">
              <Link to={`/truyen-tranh/${comic_id}`}>
                {comic ? comic.title : ""}
              </Link>{" "}
            </li>
            <li className="before:mr-3 before:text-gray-500 before:content-['/'] ">
              <Link to={`/truyen-tranh/${comic_id}/${chapter_id}`}>
                {chapter ? chapter.title : ""}{" "}
              </Link>{" "}
            </li>
          </ol>
          <h1 className="p-3 text-gray-800 text-xl font-semibold leading-6 ">
            {comic ? comic.title : ""} - {chapter ? chapter.title : ""}{" "}
            <span className="text-sm font-normal">
              (Cập nhật lúc: 21:57 27/2/2025)
            </span>
          </h1>
          <div className="text-center px-3 py-4">
            ...
            <div className="bg-sky-100 p-2 mt-3 text-sky-400 text-sm italic">
              <span>
                <i className="fa-solid fa-circle-info"></i> Sử dụng mũi tên trái
                hoặc phải để di chuyển chapter
              </span>
            </div>
        ...
        <div className="bg-white shadow rounded-sm mt-2 py-3">
          ...
          <ol className="flex gap-3 pt-5 px-3 text-gray-500">
            <li>
              <Link>Trang chủ</Link>{" "}
            </li>
            <li className="before:mr-3 before:text-gray-500 before:content-['/'] ">
              <Link>Ta là Tà Đế</Link>{" "}
            </li>
            <li className="before:mr-3 before:text-gray-500 before:content-['/'] ">
              <Link>Chương 121</Link>{" "}
            </li>
          </ol>
        </div>
        <div className="bg-white shadow rounded-sm mt-2 py-3">
